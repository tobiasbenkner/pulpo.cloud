<script lang="ts">
  ...
  const translations = {
    es: {
      ...
      noEventsDescription: 'Estamos preparando algo especial. ¡Vuelve pronto para descubrir nuestros próximos eventos!',
      price: 'Entrada',
      free: 'Gratis',
      time: 'Hora',
    ...
    en: {
      ...
      noEventsDescription: "We're preparing something special. Come back soon to discover our upcoming events!",
      price: 'Entry',
      free: 'Free',
      time: 'Time',
  ...
  const t = $derived(translations[lang as keyof typeof translations] || translations.es);
  ...
  function formatDate(dateStr: string): string {
    ...
    const formatted = date.toLocaleDateString(lang === 'en' ? 'en-US' : 'es-ES', options);
    ...
    try {
      ...
      events = (data.data || []).map((event: any) => {
        const translation = event.translations?.find(
          (t: any) => t.languages_id?.code === lang
...
<div class="max-w-4xl mx-auto">
  ...
  {:else if events.length === 0}
    <div class="bg-background rounded-2xl border border-border p-8 md:p-12 text-center">
      <div class="w-20 h-20 mx-auto mb-6 rounded-full bg-secondary/10 flex items-center justify-center">
          ...
          <div class="md:flex">
            ...
            <div class="flex-1 p-6 md:p-8 flex flex-col justify-center">
              <div class="flex flex-wrap items-center gap-3 mb-4">
                ...
                {#if event.time}
                  <span class="inline-flex items-center gap-2 bg-white/5 text-text-muted px-3 py-2 rounded-full text-sm">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <h3 class="text-2xl md:text-3xl font-bold text-white mb-3 group-hover:text-secondary transition-colors">
                {event.title}
              </h3>

              {#if event.description}
                <p class="text-text-muted text-base md:text-lg leading-relaxed mb-4 line-clamp-3">
                  {@html event.description}
                </p>
              {/if}

              <div class="flex items-center gap-4 mt-auto pt-4 border-t border-border/50">
